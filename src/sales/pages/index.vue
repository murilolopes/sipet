...
export default {
	...
	computed: {
		...
		revision() {
			let quantity = "Um pet";
			if (this.finalModel.pets.length > 1)
				quantity = `São ${this.finalModel.pets.length} pets`;
	...
	methods: {
		showTerms() {
			if (this.terms) this.saveClient();
			else this.$bvModal.show("terms-modal");
		},
		async saveClient() {
			this.$v.card.$touch();
		...
		pet_specie(pet) {
			return this.species.filter(specie => specie.value == pet.species.value)[0]
				.label;
		},
		pet_gender(pet) {
			return this.genders.filter(gender => gender.value == pet.gender.value)[0]
				.label;
		},
		pet_plan(pet) {
			return this.finalModel.plans.filter(plan => plan.id == pet.plan_id)[0]
				.name;
		}
	}
};
													...
													<div class="row" style="margin-top: 10px">
														<div class="col-12 col-lg-12">
															<b-form-group
																...
																:class="{ 'has-error': $v.card.number.$error }"
															>
																<label>
																...
																<input
																	...
																	v-model="card.number"
																	:class="{
																		'is-invalid': $v.card.number.$error
																	}"
																/>
																<div
																	v-if="$v.card.number.$error"
																	class="invalid-feedback"
																>
																	<span
																		v-if="
																			!$v.card.number.required ||
																				!$v.card.number.minLength
																		"
														...
														<div class="col-12 col-lg-12">
															<b-form-group
																label-for="cardName"
																:class="{ 'has-error': $v.card.name.$error }"
															>
																<label>
																	Nome do Titular
														...
														<div class="col-6 col-lg-6">
															<b-form-group
																...
																:class="{ 'has-error': $v.card.expiry.$error }"
															>
																<label>
																	Expira <span class="text-danger">*</span>
																</label>
																<input
																	class="form-control"
																	id="cardExpiry"
																	...
																	v-mask="'##/##'"
																	:class="{
																		'is-invalid': $v.card.expiry.$error
																	}"
																/>
																<div
																	v-if="$v.card.expiry.$error"
																	class="invalid-feedback"
																>
																	<span
																		v-if="
																			!$v.card.expiry.required ||
																				!$v.card.expiry.minLength
																		"
																		>Este campo é obrigatório.</span
																	>
																</div>
														...
														<div class="col-6 col-lg-6">
															<b-form-group
																label-for="cardCVC"
																:class="{ 'has-error': $v.card.cvc.$error }"
															>
																<label>
																...
																/>
																<div
																	v-if="$v.card.cvc.$error"
																	class="invalid-feedback"
																>
																	<span
																		v-if="
																			!$v.card.cvc.required ||
																				!$v.card.cvc.minLength
																		"
																		>Este campo é obrigatório.</span
																	>
																</div>
								...
								<div class="col-12" v-else>
									<div class="text-center">
										...
										<p class="mb-2 mx-auto">
											Parabéns pela assinatura, em poucos instantes você
											receberá um email com instruções sobre como, quando e onde
											usar seu plano e também como gerenciá-lo.
										</p>
									</div>
								</div>
							</tab-content>
